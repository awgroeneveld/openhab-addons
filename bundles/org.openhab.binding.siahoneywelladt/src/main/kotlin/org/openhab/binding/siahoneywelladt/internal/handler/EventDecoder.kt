...
class EventDecoder(val siaEventListener: SiaEventListener) {
    ...
    private fun getSiaBlock(index: Int, function: SiaFunction): SiaBlock? {
        ...
        val message = ByteArray(header.messageSize)
        val messageStartIndex = index + 2
        buffer.copyInto(message, 0, messageStartIndex, messageStartIndex + header.messageSize)
        return SiaBlock(function, header, message)
    }
